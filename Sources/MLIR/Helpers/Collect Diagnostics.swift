...
extension MLIRConfiguration {
   ...
   */
  static func collectDiagnostics<T>(minimumSeverity: Diagnostic.Severity = .remark, _ body: () -> T) -> (value: T, diagnostics: [Diagnostic]) {
    _collectDiagnostics(minimumSeverity: minimumSeverity, body)
  }
  
  static func collectDiagnostics(minimumSeverity: Diagnostic.Severity = .remark, _ body: () -> Void) -> [Diagnostic] {
